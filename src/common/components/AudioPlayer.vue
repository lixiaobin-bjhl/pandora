		...
		methods: {
			...
			init () {
				...
				// 没有vuex存储到$root中
					...
					var audios = this.$root.audios;
					if (!audios) {
						this.$root.audios = [];
					}
					this.$root.audios.push(this);
				}
			}
		},
		beforeDestroy() {
			if (this.$store) {
            	this.$store.commit('REMOVE_AUDIOS', this);
            } else {
				var audios = this.$root.audios;
				var index = audios.indexOf(this);
				this.$root.audios.splice(index, 1);
			}
			this.status = 0;
            this.audio = null;
		}
